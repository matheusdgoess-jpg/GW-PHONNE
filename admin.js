  ...
  const refresh = () => {
    ...
    refreshTimer = setTimeout(async () => {
      try {
        ...
        if (!refreshedMember?.active) { await client.auth.signOut(); location.replace('/painel-img-tech'); return; }
...
function bindEvents(){
  ...
  $('#logout').addEventListener('click',async()=>{await client.auth.signOut();location.replace('/painel-img-tech')});
  ...
  $('#password-form').addEventListener('submit',async event=>{event.preventDefault();const values=Object.fromEntries(new FormData(event.target).entries());const feedback=$('#password-feedback');if(values.newPassword.length<6){feedback.textContent='A senha precisa ter pelo menos 6 caracteres.';return}if(values.newPassword!==values.confirmPassword){feedback.textContent='As senhas digitadas não coincidem.';return}feedback.textContent='Atualizando...';const {error}=await client.auth.updateUser({password:values.newPassword});if(error){feedback.textContent='Não foi possível alterar a senha. Tente novamente.';return}event.target.reset();feedback.textContent='Senha atualizada com sucesso.';showToast('Senha atualizada','Use a nova senha no próximo acesso.')});
...
async function bootstrap() {
  ...
  if (!session) {
    location.replace('/painel-img-tech');
    return;
  }
  ...
  if (hasRole('admin')) {
    ...
    if (error && error.code !== 'PGRST116') {
      document.body.innerHTML = '<main style="min-height:100vh;display:grid;place-items:center;background:#080808;color:#fff;font-family:sans-serif;text-align:center;padding:24px"><div><h1>Não foi possível carregar o painel</h1><p>Atualize a página ou verifique a conexão com a internet.</p><a href="/painel-img-tech" style="color:#ef4038">Voltar ao login</a></div></main>';
      return;
    }
